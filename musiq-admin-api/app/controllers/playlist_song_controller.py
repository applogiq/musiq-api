...
###response of getting all user's playlist song details
def get_all_playlist_song(db):
    try:
...
def get_song_by_playlist_id(db, playlist_id):
    ...
    if playlists:
        return {"success":True,"message" : "fetched successfully","records": playlists,"total_records" :playlists}
    else:
        raise HTTPException(status_code=404, detail={"message": "couldn't fetch,check your id","success":False})
